  ...
  ws.on('message', function(data) {
    ...
    if ('id' in resp && resp.id != null) {
      ...
      if (id in self._cmdCbs) {
        self._cmdCbs[id](resp.params, resp.error);
        delete self._cmdCbs[id];
      }
    } else if ('method' in resp) {
      self.emit('notification', resp);
      self.emit(resp.method, resp.params, resp.error);
    } else {
...
ChromiumTab.prototype.close = function(successCb, errorCb) {
  var self = this;
  self.disconnect(function() {
    self._proc.closeTab(self.tab, successCb, errorCb);
  });
};
